 ...
 * 
 */
public class GlobalPlatformService implements ISO7816, APDUListener<CommandAPDU, ResponseAPDU> {
    ...
    private ArrayList<APDUListener<CommandAPDU,ResponseAPDU>> apduListeners = new ArrayList<APDUListener<CommandAPDU,ResponseAPDU>>();
    ...
    public void exchangedAPDU(APDUEvent<CommandAPDU, ResponseAPDU> e) {
        GPUtil.debug("Command  APDU: "
            + GPUtil.byteArrayToString(e.getCommandAPDU().getBytes()));
        GPUtil.debug("Response APDU: "
            + GPUtil.byteArrayToString(e.getResponseAPDU().getBytes()));
    }
